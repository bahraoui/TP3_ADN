...
int main(int argc, char *argv[])
    ...
    int ret;
    Cellule **tabh;
    long long int taille_tab_h = 320000;

    /* ALLOCATION DE LA TABLE DE HACHAGE                                          */
    tabh = (Cellule **)calloc(taille_tab_h,sizeof(Cellule*));
    ...
    while(fgets(line,100,f))
        ...
        if(ret==1)
            ...
            printf("%s\n",str);
    #endif
/* Ajout du mot str dans la structure de données                              */
/******************************************************************************/
/* A VOUS DE COMPLETER ICI                                                    */
/******************************************************************************/
                long long int hash = hachage(str) % taille_tab_h;
                tabh[hash] = ajouteEnTeteLC(tabh[hash], str);
                if(strcmp(str, "rj") == 0) {
                    printf("rj exist...\n");
                }
            }
...
/******************************************************************************/
        ...
        unsigned long long int hash = hachage(str) % taille_tab_h;
        int exist = chercheMotLC(tabh[hash], str);
...
/*             printf("%s OK\n",str); */
    ...
    int n = chemins(m, racine, tabh, taille_tab_h);
    printf("chemins : %d\n", n);    
    attendre_clic();
    fermer_fenetre();
